            ...
            if rect!=None:
                ...
                if eye_aspect_ratio < EYE_DROWSINESS_THRESHOLD:
                    ...
                    if time.time()-eye_start_time >= EYE_DROWSINESS_INTERVAL:
                        ...
                        cv2.imwrite(os.path.join(path,'image.jpg'),frame) # capture Image
                        while proc_count == 0 and mail_count <5: #send email
                            proc = subprocess.Popen([sys.executable,'sendalert.py'])
                            proc.poll()
                            proc_count += 1
                            mail_count += 1
